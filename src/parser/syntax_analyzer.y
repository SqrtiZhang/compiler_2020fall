...
%token <node> ADD SUB MUL DIV LT LTE GT GTE EQ NEQ ASSIGN
...
%type <node> expression var simple-expression additive-expression term factor integer float call
...
var-declaration : type-specifier IDENTIFIER SEMICOLON{
    $$ = node("var-declaration", 3, $1, new_syntax_tree_node($2), $3);
} | type-specifier IDENTIFIER LBRACKET INTEGER RBRACKET SEMICOLON{
    syntax_tree_node * temp = new_syntax_tree_node("");
    sprintf(temp->name, "%d", $4);
    $$ = node("var-declaration", 6, $1, new_syntax_tree_node($2), $3, temp, $5, $6);
}
...
fun-declaration : type-specifier IDENTIFIER LPARENTHESE params RPARENTHESE compound-stmt{
    $$ = node("fun-declaration", 6, $1, new_syntax_tree_node($2), $3, $4, $5, $6);
}
...
param : type-specifier IDENTIFIER {
    $$ = node("param", 2, $1, new_syntax_tree_node($2));
} | type-specifier IDENTIFIER ARRAY{
    $$ = node("param", 3, $1, new_syntax_tree_node($2), $3);
}
...
statement-list : statement-list statement{
    ...
    $$ = node("statement-list", 1, new_syntax_tree_node("epsilon"));
}
...
var : IDENTIFIER{
    $$ = node("var", 1, new_syntax_tree_node($1));
} | IDENTIFIER LBRACKET expression RBRACKET{
    $$ = node("var", 4, new_syntax_tree_node($1), $2, $3, $4);
}
...
integer : INTEGER{
    syntax_tree_node * temp = new_syntax_tree_node("");
    sprintf(temp->name, "%d", $1);
    $$ = node("integer", 1, temp);
}

float : FLOATPOINT{
    $$ = node("float", 1, new_syntax_tree_node($1));
}

call : IDENTIFIER LPARENTHESE args RPARENTHESE{
    $$ = node("call", 4, new_syntax_tree_node($1), $2, $3, $4);
}

args : arg-list{
    $$ = node("args", 1, $1);
} | {
    $$ = node("args", 1, new_syntax_tree_node("epsilon"));
}
